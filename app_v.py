    ...
    if tipo_envio == "📍 Pedido Local":
        ...
        if not route_template_path.exists():
            ...
            st.download_button(
                ...
                disabled=bool(route_missing_fields),
                help=(
                    "Completa al menos cliente, folio y calle para descargar la hoja de ruta."
                    if route_missing_fields
                    else "Descarga la hoja de ruta local con la información capturada."
                ),
            )
            if route_missing_fields:
                st.caption("Faltan campos para habilitar la descarga: " + ", ".join(route_missing_fields))
            else:
                st.caption(
