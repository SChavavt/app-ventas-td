...
def pedido_referencia_publica(cliente: object, folio: object) -> str:
    """Construye una referencia legible para UI sin exponer ID_Pedido interno."""
    cliente_txt = str(cliente or "").strip() or "Cliente sin nombre"
    folio_txt = clean_folio_for_ui(folio) or "Sin folio"
    return f"{cliente_txt} (Folio: {folio_txt})"
...
with tab1:
    ...
    if should_process_submission:
        ...
        try:
            ...
            set_pedido_submission_status(
                "success",
                f"✅ Pedido registrado correctamente para {pedido_referencia_publica(cliente, folio_factura_input)}{id_vendedor_segment}.",
...
with tab2:
                        ...
                        if not new_modificacion_surtido_input.strip():
                            ...
                            try:
                                ...
                                if changes_made:
                                    ...
                                    st.session_state["last_updated_order_cliente"] = str(selected_row_data.get("Cliente", "")).strip()
                                    st.session_state["last_updated_order_folio"] = str(selected_row_data.get("Folio_Factura", "")).strip()
    ...
    if (
        ...
        with message_placeholder_tab2.container():
            st.success(
                f"🎉 ¡Cambios guardados con éxito para **{pedido_referencia_publica(st.session_state.get('last_updated_order_cliente', ''), st.session_state.get('last_updated_order_folio', ''))}**!"
            )
            if st.button("Aceptar", key="ack_mod_success"):
                for state_key in (
                    ...
                    "last_updated_order_cliente",
                    "last_updated_order_folio",
                ):
        ...
        if (
            ...
            st.toast(f"✅ Pedido actualizado: {pedido_referencia_publica(st.session_state.get('last_updated_order_cliente', ''), st.session_state.get('last_updated_order_folio', ''))}", icon="📦")
