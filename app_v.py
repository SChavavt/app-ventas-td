...
with tab1:
    ...
    with st.form(key=f"new_pedido_form_{form_nonce}", clear_on_submit=False):
        ...
        auto_route_generated_at = st.session_state.get(LOCAL_ROUTE_GENERATED_AT_KEY, "")
        ...
        if tipo_envio == "📍 Pedido Local" and auto_route_filename and auto_route_file_data:
            st.success("✅ Hoja de ruta generada y adjuntada automáticamente al pedido.")
            st.markdown("##### 🤖 Adjuntos automáticos del pedido")
            timestamp_label = auto_route_generated_at or "Sin fecha"
            st.markdown(
                "\n".join(
                    [
                        f"- 📎 `{auto_route_filename}`",
                        "- Estado: listo para subirse con el pedido",
                        "- Origen: hoja de ruta generada automáticamente",
    ...
    if tipo_envio == "📍 Pedido Local":
        ...
        if confirm_route_button:
            generated_route_name = st.session_state.get(LOCAL_ROUTE_GENERATED_FILENAME_KEY, "")
            if generated_route_name:
                st.success(
                    "✅ Datos de hoja de ruta confirmados. "
                    f"El archivo `{generated_route_name}` quedó adjuntado automáticamente al pedido."
                )
            else:
                st.warning("⚠️ Los datos se confirmaron, pero la hoja de ruta no pudo adjuntarse automáticamente.")
        ...
        if confirmed_route_payload:
            ...
            if not route_template_path.exists():
                ...
                generated_route_name = st.session_state.get(LOCAL_ROUTE_GENERATED_FILENAME_KEY, "")
                generated_route_at = st.session_state.get(LOCAL_ROUTE_GENERATED_AT_KEY, "")
                if generated_route_name:
                    st.info(
                        "📎 Hoja de ruta lista como adjunto automático: "
                        f"`{generated_route_name}`"
                        + (f" · Generada: {generated_route_at}" if generated_route_at else "")
                    )
