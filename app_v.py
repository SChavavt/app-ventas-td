...
def get_local_delivery_slot(turno_local: str) -> str:
    ...
    if turno_normalizado in {"🌵 Saltillo", "📦 Pasa a Bodega"}:
...
def get_default_local_route_hour_for_turno(turno_local: str) -> str:
    """Infer the route delivery hour from the current/original turno."""
    return get_local_delivery_slot(turno_local)


def get_subtipo_local_excel_value(subtipo_local: str) -> str:
    """Normalize local shift label for Excel persistence."""
    return str(subtipo_local or "").strip()


LOCAL_TURNO_COMBINADO_IDS: set[str] = set()
TAB1_DUAL_VIEW_IDS = {"ALEJANDRO38", "ANGIE27", "CECILIA94", "CARITO82", "GLORIA53", "KAREN58", "NORMA12"}
TAB1_CDMX_ONLY_VIEW_IDS = set()
...
def get_blocked_local_shifts_for_options(
    delivery_date: date,
    local_shift_options: list[str],
) -> set[str]:
    """Return closed shifts that apply to the currently visible local options."""
    blocked_shifts = get_blocked_local_shifts_for_date(delivery_date)
    if not blocked_shifts:
        return set()

    visible_options = {str(option or "").strip() for option in local_shift_options}
    applicable_blocked_shifts = {
        shift for shift in blocked_shifts if shift in visible_options
    }

    return applicable_blocked_shifts
...
def build_daily_deposit_report_sheet(
    ...
    for indice, (_, registro) in enumerate(df_filtrado.iterrows()):
        ...
        worksheet[f"F{fila_actual}"] = str(registro.get("Banco_Destino_Pago") or "").strip()
        worksheet[f"G{fila_actual}"] = monto_valor
        worksheet[f"H{fila_actual}"] = monto_valor
        worksheet[f"J{fila_actual}"] = comentario_limpio
...
def worksheet_to_dataframe_safe(worksheet, retries: int = 3, base_delay: float = 0.6) -> pd.DataFrame:
    """Convierte una hoja de Google Sheets a DataFrame manejando reintentos y encabezados duplicados."""
...
def obtener_resumen_guias_vendedor(id_vendedor_norm: str, refresh_token: float | None = None) -> dict:
    ...
    for col in ["id_vendedor", "Adjuntos_Guia", "Hoja_Ruta_Mensajero", "Cliente", "ID_Pedido", "Folio_Factura", "Completados_Limpiado", "Hora_Registro"]:
        if col not in df_ped.columns:
            df_ped[col] = ""

    for col in ["id_vendedor", "Hoja_Ruta_Mensajero", "Adjuntos_Guia", "Cliente", "ID_Pedido", "Folio_Factura", "Completados_Limpiado", "Hora_Registro"]:
    ...
    for df_tmp in (df_ped, df_casos):
        try:
            df_tmp["Hora_Registro_dt"] = pd.to_datetime(df_tmp["Hora_Registro"], errors="coerce", format="mixed")
        except (TypeError, ValueError):
            df_tmp["Hora_Registro_dt"] = pd.to_datetime(df_tmp["Hora_Registro"], errors="coerce")

    cutoff_12h = datetime.now() - timedelta(hours=12)
    df_ped = df_ped[df_ped["Hora_Registro_dt"].notna() & (df_ped["Hora_Registro_dt"] >= cutoff_12h)].copy()
    df_casos = df_casos[df_casos["Hora_Registro_dt"].notna() & (df_casos["Hora_Registro_dt"] >= cutoff_12h)].copy()
...
with st.expander("🧮 Calculadora de descuento", expanded=False):
    ...
    components.html(
        """
        <script>
          const decimalInputSelector = [
            'input[aria-label="Precio a obtener"]',
            'input[placeholder^="Precio regular"]'
          ].join(',');

          const sanitizeDecimalValue = (raw) => {
          ...
          const setupDecimalInput = (input) => {
            if (!input || input.dataset.discountDecimalBound === "1") return;
            input.dataset.discountDecimalBound = "1";
            input.setAttribute("inputmode", "decimal");
            input.setAttribute("autocomplete", "off");

            input.addEventListener("input", (event) => {
              const current = event.target.value || "";
              const sanitized = sanitizeDecimalValue(current);
          ...
          const bindAllDecimalInputs = () => {
            document.querySelectorAll(decimalInputSelector).forEach(setupDecimalInput);
          };

          bindAllDecimalInputs();
          const observer = new MutationObserver(bindAllDecimalInputs);
...
def render_caso_especial(row, show_material: bool = True):
    ...
    if show_material and __has(row.get("Material_Devuelto","")):
        ...
        if not material_has_structured_format:
            ...
            material_rows = parse_material_lines(material_text)
            if material_rows:
...
with tab1:
    ...
    if usa_logica_local:
        if tipo_envio in {"📍 Pedido Local", UBER_TIPO_ENVIO}:
            ...
            blocked_shifts_for_date = get_blocked_local_shifts_for_options(
                fecha_entrega_local_ui,
                local_shift_options,
            )
            if blocked_shifts_for_date:
                local_shift_options = [opt for opt in local_shift_options if opt not in blocked_shifts_for_date]
                fecha_cierre_txt = fecha_entrega_local_ui.strftime("%d/%m/%Y") if isinstance(fecha_entrega_local_ui, date) else str(fecha_entrega_local_ui)
                turnos_cerrados_txt = ", ".join(sorted(blocked_shifts_for_date))
                st.info(f"ℹ️ Para la fecha {fecha_cierre_txt} los siguientes turnos están cerrados: {turnos_cerrados_txt}.")
        ...
        if usa_logica_local:
            if usa_hoja_ruta_local and not is_local_pasa_bodega:
                ...
                if is_devolucion_local:
                    ...
                    local_route_dia_entrega = ""
                    if not str(local_route_hora_entrega or "").strip():
                        hora_manual_actual = str(st.session_state.get("local_route_hora_entrega_manual", "") or "").strip()
                        local_route_hora_entrega = resolve_local_delivery_slot(subtipo_local, hora_manual_actual)
                    st.session_state["local_route_dia_entrega"] = ""
        ...
        if filtered_orders.empty:
            ...
            if selected_option_key and selected_option_key != order_placeholder_option:
                ...
                if option_changed and tab2_local_order:
                    ...
                    st.session_state[f"{tab2_route_prefix}_hora_entrega"] = get_default_local_route_hour_for_turno(
                        turno_local_default_tab2,
                    )
                    st.session_state[f"{tab2_route_prefix}_last_turno_for_hora"] = turno_local_default_tab2
                    ...
                    if apply_local_route_update:
                        ...
                        if blocked_shifts_tab2:
                            ...
                            keep_original_turno_tab2 = st.checkbox(
                                f"Mantener el turno actual del pedido original: {turno_original_display_tab2}",
                                key="tab2_keep_original_turno_for_closed_shift",
                                help=(
                                    f"Márcalo para conservar {turno_original_display_tab2} "
                        ...
                        tab2_hora_entrega_key = f"{tab2_route_prefix}_hora_entrega"
                        tab2_last_turno_hora_key = f"{tab2_route_prefix}_last_turno_for_hora"
                        tab2_hora_default = get_local_delivery_slot(tab2_turno_local)
                        if not str(st.session_state.get(tab2_hora_entrega_key, "") or "").strip():
                            st.session_state[tab2_hora_entrega_key] = tab2_hora_default
                        if str(st.session_state.get(tab2_last_turno_hora_key, "") or "").strip() != str(tab2_turno_local or "").strip():
                            st.session_state[tab2_hora_entrega_key] = tab2_hora_default
                        st.session_state[tab2_last_turno_hora_key] = str(tab2_turno_local or "").strip()

                        tab2_local_route_hora_entrega = st.text_input(
                            "🕒 HORA DE ENTREGA",
                            key=tab2_hora_entrega_key,
                            placeholder="Ej. 10 am a 2 pm, 3 pm a 7 pm u Horario Libre",
                            help=(
                                "Este es el horario que se mandará a la hoja de ruta. "
                                "Se precarga desde el pedido/turno actual, pero puedes cambiarlo manualmente."
                            ),
                        ).strip()
                        if not tab2_local_route_hora_entrega:
                            tab2_local_route_hora_entrega = tab2_hora_default
                            st.session_state[tab2_hora_entrega_key] = tab2_hora_default
                        if tab2_turno_local not in ["☀️ Local Mañana", "🌙 Local Tarde"]:
                            st.warning("⚠️ La hoja de ruta asigna horario automático para ☀️ Local Mañana (10 am a 2 pm) y 🌙 Local Tarde (3 pm a 7 pm). Para otros turnos se usará el texto del turno seleccionado o el horario que captures.")
                        ...
                        tab2_route_payload = build_local_route_payload(
                            ...
                            hora_entrega_manual=tab2_local_route_hora_entrega,
                        )
                        st.caption(
                ...
                if filtered_casos.empty:
                    ...
                    if selected_case:
                        ...
                        with st.form(key=f"{edit_key_base}_form", clear_on_submit=False):
                            ...
                            estado_caso_options = ["", "✏️ Modificación", "Pendiente", "En proceso", "Cerrado"]
                            estado_actual_caso = str(case_row.get("Seguimiento", "") or "").strip()
                            if estado_actual_caso and estado_actual_caso not in estado_caso_options:
                                estado_caso_options.append(estado_actual_caso)
                            seguimiento_caso = st.selectbox(
                                "📌 Seguimiento",
                                estado_caso_options,
                                index=estado_caso_options.index(estado_actual_caso) if estado_actual_caso in estado_caso_options else 0,
                                key=f"{edit_key_base}_seguimiento",
                            )
...
with tab5:
    ...
    current_refresh_token = st.session_state.get("guias_refresh_token")
    cache_payload = st.session_state.get("tab5_guias_dataset_cache")
    use_cached_df = (
        isinstance(cache_payload, dict)
        and cache_payload.get("refresh_token") == current_refresh_token
        and isinstance(cache_payload.get("df"), pd.DataFrame)
        and not refresh_pressed
    )

    if use_cached_df:
        df_guias = cache_payload["df"].copy()
    else:
        try:
            df_guias = cargar_datos_guias_unificadas(current_refresh_token)
        except Exception as e:
            st.error(f"❌ Error al cargar datos de guías: {e}")
            df_guias = pd.DataFrame()
        st.session_state["tab5_guias_dataset_cache"] = {
