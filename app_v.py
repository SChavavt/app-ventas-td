...
def load_credentials_from_file(file_path):
    ...
    try:
        ...
        st.error(f"❌ Error: El archivo de credenciales '{file_path}' no fue encontrado. Asegúrate de que el nombre sea correcto y esté en la misma carpeta que 'app_vendedores.py'.")
        st.stop()
...
def get_gspread_client(credentials_json):
    """Autentica y retorna un cliente de gspread."""
...
try:
    ...
    g_spread_client = get_gspread_client(creds_json)
    s3_client = get_s3_client() # NEW: Initialize S3 client
except Exception as e:
    st.error(f"❌ Error general al autenticarse o inicializar clientes de Google Sheets/S3: {e}")
    st.info("ℹ️ Asegúrate de que el archivo de credenciales de Google esté en la raíz de tu proyecto, que las APIs de Sheets estén habilitadas en Google Cloud, y que tus credenciales de AWS S3 sean correctas en `.streamlit/secrets.toml`.")
    st.stop()
