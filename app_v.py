...
def resolver_nombre_y_enlace_busqueda(valor, etiqueta_fallback):
    ...
    if parsed.scheme and parsed.netloc:
        ...
        enlace = get_s3_file_download_url_busqueda(s3_client, valor)
    ...
    if mod_txt or mod_urls:
        ...
        if mod_urls:
            ...
            for u in mod_urls:
                ...
                tmp = get_s3_file_download_url_busqueda(s3_client, u)
        ...
        if modo_busqueda == "🧑 Por cliente/factura":
            ...
            for _, row in df_pedidos.iterrows():
                ...
                resultados.append({
                    ...
                    "Folio_Factura_Refacturada": str(row.get("Folio_Factura_Refacturada", "")).strip(),
                    "Coincidentes": [],
                    "Comprobantes": [(f["Key"], get_s3_file_download_url_busqueda(s3_client, f["Key"])) for f in comprobantes],
                    "Facturas": [(f["Key"], get_s3_file_download_url_busqueda(s3_client, f["Key"])) for f in facturas],
                    "Otros": [(f["Key"], get_s3_file_download_url_busqueda(s3_client, f["Key"])) for f in otros],
                })
            ...
            for _, row in df_pedidos.iterrows():
                ...
                for archivo in archivos_validos:
                    ...
                    if coincide:
                        ...
                        archivos_coincidentes.append((key, get_s3_file_download_url_busqueda(s3_client, key)))
