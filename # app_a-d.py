...
try:
    ...
    AWS_REGION = st.secrets["aws_region"] # Mantener AWS_REGION para consistencia con la función get_s3_client
...
def load_credentials_from_file(file_path):
    ...
    try:
        if not os.path.exists(file_path):
            st.error(f"❌ Error: El archivo de credenciales de Google Sheets '{file_path}' no fue encontrado. Asegúrate de que el nombre sea correcto y esté en la misma carpeta que 'app.py'.")
            st.stop()
...
try:
    google_creds_dict = load_credentials_from_file(SERVICE_ACCOUNT_FILE)
...
def check_and_update_demorados(df_to_check, worksheet, headers): # Añadir 'headers'
    ...
    if updates_to_perform:
        if batch_update_gsheet_cells(worksheet, updates_to_perform):
            ...
            st.cache_resource.clear() # Invalidar caché después de la actualización por lotes
...
def mostrar_pedido(df, idx, row, orden, origen_tab, current_main_tab_label, worksheet, headers): # Añadir 'headers'
    ...
    with st.container():
        ...
        def update_surtidor_callback(current_idx, current_gsheet_row_index, current_surtidor_key):
        ...
        col_surtidor.text_input(
            ...
            args=(idx, gsheet_row_index, surtidor_key)
        )
        ...
        if col_print_btn.button("🖨 Imprimir", key=f"print_button_{row['ID_Pedido']}_{origen_tab}", disabled=disabled_if_completed):
            ...
            st.rerun() # Keep rerun here if expanding attachments and updating status should rerender immediately
        ...
        if col_complete_btn.button("🟢 Completar", key=f"done_{row['ID_Pedido']}_{origen_tab}", disabled=disabled_if_completed):
            ...
            if surtidor_final:
                ...
                if batch_update_gsheet_cells(worksheet, updates_for_complete_button):
                    ...
                    st.rerun() # Keep rerun here as completing an order likely means it moves out of the current view
                else:
        ...
        def update_notas_callback(current_idx, current_gsheet_row_index, current_notas_key):
        ...
        st.text_area(
            ...
            args=(idx, gsheet_row_index, notas_key)
        )
