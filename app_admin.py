...
def clean_folio_for_ui(value) -> str:
    """Muestra folios capturados post-registro sin el prefijo interno *."""
    folio = normalize_folio_factura(value)
    return folio[1:].strip() if folio.startswith("*") else folio


def is_devolucion_case_row(row: pd.Series) -> bool:
    """Detecta devoluciones usando Tipo_Caso o Tipo_Envio según lo disponible."""
...
def extract_pedido_folder_candidates(selected_pedido_data, pedido_id: str) -> list[str]:
    ...
    if selected_pedido_data is None or not hasattr(selected_pedido_data, "get"):
        return candidates
...
with tab2:
    ...
    if df_confirmados_guardados.empty:
        ...
        if nombres_credito_normalizados and "Cliente" in df_tabla_mostrar.columns:
            ...
            try:
                # Streamlit + Styler requieren índices/columnas manejables; reforzamos para evitar excepciones.
                df_tabla_mostrar_safe = df_tabla_mostrar.copy()
                df_tabla_mostrar_safe.columns = [
                    str(col) if col is not None else "" for col in df_tabla_mostrar_safe.columns
                ]
                if len(set(df_tabla_mostrar_safe.columns)) != len(df_tabla_mostrar_safe.columns):
                    cols_seen: dict[str, int] = {}
                    cols_unique: list[str] = []
                    for col in df_tabla_mostrar_safe.columns:
                        count = cols_seen.get(col, 0)
                        cols_unique.append(col if count == 0 else f"{col}__{count}")
                        cols_seen[col] = count + 1
                    df_tabla_mostrar_safe.columns = cols_unique
                df_tabla_mostrar_safe = df_tabla_mostrar_safe.reset_index(drop=True)
                mask_tabla_credito = mask_tabla_credito.reset_index(drop=True)

                st.dataframe(
                    df_tabla_mostrar_safe.style.apply(_underline_row_if_credito, axis=1),
                    use_container_width=True,
                    hide_index=True,
                )
            except Exception:
                st.warning("⚠️ No se pudo aplicar subrayado visual en tabla, se muestra la tabla sin estilo.")
                st.dataframe(
                    df_tabla_mostrar,
                    use_container_width=True,
                    hide_index=True,
...
with tab3, suppress(StopException):
    ...
    needed_cols = [
        ...
        "Numero_Serie","Fecha_Compra","Seguimiento","Comentario_Gerente", GUIAS_DEVOLUCION_COL
    ]
    ...
    def render_caso_especial(row):
        ...
        comentario_gerente = __s(row.get("Comentario_Gerente", ""))
        if __has(comentario_gerente):
            st.markdown("**🟨 Comentario de Gerencia:**")
            st.warning(comentario_gerente)
        ...
        if ok_all and is_dev:
            ...
            try:
                with st.spinner("Generando formato de devolución..."):
                    ...
                    # Nota: se realiza un roundtrip de normalización para reducir
                    # avisos de "contenido no legible" en Microsoft Word.
                    out_buffer = BytesIO()
                    doc.save(out_buffer)
                    out_buffer.seek(0)
                    normalized_doc = Document(out_buffer)
                    normalized_buffer = BytesIO()
                    normalized_doc.save(normalized_buffer)
                    normalized_buffer.seek(0)

                    cliente_archivo = str(row.get("Cliente", "") or "").strip() or "Cliente"
                    cliente_archivo = re.sub(r"[^A-Za-z0-9ÁÉÍÓÚáéíóúÑñ _-]", "", cliente_archivo).strip()
                    cliente_archivo = re.sub(r"\s+", "_", cliente_archivo) or "Cliente"

                    st.download_button(
                        label="📄 Descargar Formato de Devolución",
                        data=normalized_buffer,
                        file_name=f"Formato_Devolucion_{cliente_archivo}.docx",
