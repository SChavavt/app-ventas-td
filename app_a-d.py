...
# ELIMINADO: SERVICE_ACCOUNT_FILE ya no es necesario, leeremos de secrets
...
# NEW: Función para obtener el cliente de gspread usando credenciales de Streamlit secrets
@st.cache_resource
def get_google_sheets_client():
    ...
    try:
        ...
        st.error("❌ Error: Las credenciales de Google Sheets no se encontraron en Streamlit secrets. Asegúrate de que la clave 'google_credentials' exista en tu archivo .streamlit/secrets.toml y sea un JSON válido.")
        st.info("ℹ️ El formato en secrets.toml debe ser `google_credentials = \"{\"type\": \"service_account\", ...}\" ` (la cadena JSON debe estar entre comillas dobles).")
        st.stop()
    except json.JSONDecodeError:
        st.error("❌ Error: El valor de 'google_credentials' en Streamlit secrets no es un JSON válido. Revisa su formato.")
        st.stop()
    except Exception as e:
        st.error(f"❌ Error al autenticar con Google Sheets: {e}")
        st.info("ℹ️ Verifica que las APIs de Google Sheets y Drive estén habilitadas para tu proyecto de Google Cloud.")
        st.stop()
...
try:
    g_spread_client = get_google_sheets_client() # Usar la nueva función
    s3_client = get_s3_client()
except Exception as e:
    st.error(f"❌ Error general al autenticarse o inicializar clientes: {e}")
    st.info("ℹ️ Asegúrate de que tus credenciales de Google Sheets estén en `st.secrets['google_credentials']` y tus credenciales de AWS S3 en `st.secrets['aws']`.")
    st.stop()
